...
func TestCleanJSONSchemaForGemini_PropertyNamesConversion(t *testing.T) {
	// propertyNames is used to validate object property names (e.g., must match a pattern)
	// Instead of removing it, we convert it to patternProperties for schema compatibility
	// with systems like Factory Droid CLI with MCP that don't support propertyNames
	input := `{
		"type": "object",
		"properties": {
			"metadata": {
	...
	// Verify propertyNames is converted (removed from output)
	if strings.Contains(result, "propertyNames") {
		t.Errorf("propertyNames keyword should be converted and removed, got: %s", result)
	}

	// Verify the description hint about property name constraint is added
	if !strings.Contains(result, "Property names must match") {
		t.Errorf("Expected description hint about property name pattern, got: %s", result)
	}
}

func TestCleanJSONSchemaForGemini_PropertyNamesConversion_Nested(t *testing.T) {
	...
	if strings.Contains(result, "propertyNames") {
		t.Errorf("Nested propertyNames should be converted and removed, got: %s", result)
	}

	// Verify the description hint is added for nested conversion
	if !strings.Contains(result, "Property names must match") {
		t.Errorf("Expected description hint about property name pattern in nested schema, got: %s", result)
	}
}

func TestCleanJSONSchemaForGemini_PropertyNamesConversion_DefaultPattern(t *testing.T) {
	// Test propertyNames without a pattern (should use default .* pattern)
	input := `{
		"type": "object",
		"propertyNames": {
			"type": "string"
	...
	if strings.Contains(result, "propertyNames") {
		t.Errorf("propertyNames should be converted and removed, got: %s", result)
	}

	// Should use default pattern .*
	if !strings.Contains(result, "Property names must match: .*") {
		t.Errorf("Expected default pattern .* in hint, got: %s", result)
	}
}
