...
func CleanJSONSchemaForGemini(jsonStr string) string {
	...
	jsonStr = convertPropertyNamesToPatternProperties(jsonStr)
...
// convertPropertyNamesToPatternProperties transforms propertyNames to patternProperties.
// This provides schema compatibility for systems that don't support propertyNames keyword.
...
//	}
func convertPropertyNamesToPatternProperties(jsonStr string) string {
	paths := findPaths(jsonStr, "propertyNames")
	sortByDepth(paths)

	for _, p := range paths {
		propertyNamesVal := gjson.Get(jsonStr, p)
		if !propertyNamesVal.Exists() {
			continue
		}

		parentPath := trimSuffix(p, ".propertyNames")

		// Extract pattern from propertyNames if it exists
		pattern := propertyNamesVal.Get("pattern").String()
		if pattern == "" {
			// If no pattern, use a default that matches any string
			pattern = ".*"
		}

		// Get the existing additionalProperties type if present
		additionalPropsPath := joinPath(parentPath, "additionalProperties")
		additionalProps := gjson.Get(jsonStr, additionalPropsPath)

		// Build the patternProperties value type
		valueType := `{"type":"string"}`
		if additionalProps.IsObject() {
			// Use the existing additionalProperties schema as the value type
			valueType = additionalProps.Raw
		} else if additionalProps.Exists() && additionalProps.Type == gjson.True {
			// additionalProperties: true means any type is allowed
			valueType = `{}`
		}

		// Set patternProperties with the extracted pattern
		patternPropsPath := joinPath(parentPath, "patternProperties")
		escapedPattern := escapeGJSONPathKey(pattern)
		patternValuePath := joinPath(patternPropsPath, escapedPattern)
		jsonStr, _ = sjson.SetRaw(jsonStr, patternValuePath, valueType)

		// Set additionalProperties to false to enforce the pattern
		jsonStr, _ = sjson.Set(jsonStr, additionalPropsPath, false)

		// Remove the original propertyNames
		jsonStr, _ = sjson.Delete(jsonStr, p)

		// Add a hint about the property name constraint
		jsonStr = appendHint(jsonStr, parentPath, fmt.Sprintf("Property names must match: %s", pattern))
	}
	return jsonStr
}
...
func removeUnsupportedKeywords(jsonStr string) string {
	keywords := append(unsupportedConstraints,
		...
		"patternProperties", // Remove patternProperties after conversion hints are added
		// Note: propertyNames is handled by convertPropertyNamesToPatternProperties
	)
