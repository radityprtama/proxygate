...
import (
	...
	copilotauth "github.com/radityprtama/proxygate/v6/internal/auth/copilot"
...
type GitHubCopilotExecutor struct {
	cfg   *config.Config
	mu    sync.RWMutex
	cache map[string]*cachedCopilotAPIToken
}

// cachedCopilotAPIToken stores a cached Copilot API token with its expiry.
type cachedCopilotAPIToken struct {
	token     string
	expiresAt time.Time
}
...
func NewGitHubCopilotExecutor(cfg *config.Config) *GitHubCopilotExecutor {
	return &GitHubCopilotExecutor{
		cfg:   cfg,
		cache: make(map[string]*cachedCopilotAPIToken),
	}
}

// Identifier implements ProviderExecutor.
func (e *GitHubCopilotExecutor) Identifier() string {
	return githubCopilotAuthType
}
...
func (e *GitHubCopilotExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if detail := parseOpenAIUsage(data); detail.TotalTokens > 0 {
...
func (e *GitHubCopilotExecutor) ensureAPIToken(ctx context.Context, auth *cliproxyauth.Auth) (string, error) {
	...
	e.mu.Lock()
	e.cache[accessToken] = &cachedCopilotAPIToken{
